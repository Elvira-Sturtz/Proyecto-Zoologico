...
itinerarioCtrl.getItinerarios = async (req, res) => {
  try {
    const itinerarios = await Itinerario.find().populate("zonas");
...
itinerarioCtrl.getItinerarioById = async (req, res) => {
  try {
    const itinerario = await Itinerario.findById(req.params.id).populate(
      "zonas",
    );
...
itinerarioCtrl.updateItinerario = async (req, res) => {
  try {
    const { codigo, duracion, longitud, maxVisitantes, zonas } = req.body;
